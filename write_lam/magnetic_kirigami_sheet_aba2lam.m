% =============================================================================
%              MAGNETIC KIRIGAMI SHEET LAMMPS DATA FILE GENERATOR
% =============================================================================
% Generates LAMMPS data file for a magnetic kirigami sheet.
% Computes dipole moments from a deformed reference configuration.
%
...
lamfile = 'kirigami_magnetic_sheet_d11_t025_m03_E15_r40_a90_Br106mT.lam';
xyzfile = 'stretched_kirigami_120.xyz';  % Deformed reference
...
Br_SI = 0.106;                         % Remanent magnetization (T)

% Geometric parameters
t = 0.25;                              % Film thickness
a = 0.4;                               % Lattice size (for diameter)
...
figure;
triplot(T, P(:,1), P(:,2), 'r');
title('Kirigami Sheet Mesh (Magnetized)');
xlabel('X'); ylabel('Y');
axis equal;


% =============================================================================
%                       RECOMPUTE AREAS & PROPERTIES
% =============================================================================
...
% Center atom masses (scaled up to stabilize simulations)
cmass   = rho * Area * t * 4.0;
acmass  = sum(cmass) / NT;
...
% =============================================================================
%                   LOAD DEFORMED CONFIGURATION & COMPUTE DIPOLES
% =============================================================================

data = importdata(xyzfile, ' ', 9);
ll   = data.data(:, end);
nt   = length(ll);
xyz  = zeros(nt, 3);
xyz(ll, :) = data.data(:, 2:4);
clear data

% Coordinate of each node in the triangle
xyz1 = xyz(T(:,1), :);
xyz2 = xyz(T(:,2), :);
xyz3 = xyz(T(:,3), :);

% Bonds after deformation
r12 = xyz2 - xyz1;
r13 = xyz3 - xyz1;

% Bonds before deformation
R12 = P2 - P1;
R13 = P3 - P1;

% Compute dipole moments from deformed configuration
mu_c = zeros(NT, 3);
M  = Br_SI / mu0 / M_scale;  
...
for ii = 1:NT
    ...
    % Rotation matrix from deformed to undeformed: R = Rot * r
    R = [R12_l', R13_l', N_l'] * inv([r12_l', r13_l', n_l']);
    
    muc1 = [0, 0, 1];
    muc0 = R * muc1';
    
    mu_c(ii, :) = (M * Area(ii) * t) * muc0' / norm(muc0);
end
